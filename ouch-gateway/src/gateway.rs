...
pub fn read(buf: Vec<u8>, state: AppState) {

      let inbound = inbound::parse_enter_order(&buf[1..]).unwrap();

        let res = match inbound {
            InBoundResponse::Enter(add_order) => matching_engine::ouch::add_order(state, 
                add_order.symbol, 1 , 
                add_order.side, add_order.price, add_order.qty, add_order.time_in_force),
